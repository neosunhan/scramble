...
function uploadQuotes() {
    ...
    for (let i = 0; i < 10; i++) {
        generated_quotes[i.toString()] = "test quote " + i.toString();
    }
    window.db_set(window.db_ref(window.database, 'quotes'), generated_quotes);
}
...
function getNextQuote() {
    window.db_get(window.db_child(window.db_ref(window.database), `quotes/${quote_num.toString()}`)).then((snapshot) => {
        if (snapshot.exists()) {
